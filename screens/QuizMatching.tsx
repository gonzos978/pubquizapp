...
export interface QuizMatchingHandle {
    pauseTimer: () => void;
    resumeTimer: () => void;
}

// eslint-disable-next-line react/display-name
const QuizMatching = forwardRef<QuizMatchingHandle, QuizMatchingProps>(
    ({ route }, ref) => {
        const { data, leftItems, rightItems, reviewMode, playerAnswers, sfs } =
            route.params;
        ...
        useImperativeHandle(ref, () => ({
            pauseTimer: () => setIsPausedTimer(true),
            resumeTimer: () => setIsPausedTimer(false),
        }));

        // Expose pause/resume via route callback (for JoinScreen)
        useEffect(() => {
            if (route.params.setPauseControl) {
                route.params.setPauseControl({
                    pauseTimer: () => setIsPausedTimer(true),
                    resumeTimer: () => setIsPausedTimer(false),
                });
            }
        ...
        // Reset timer and state on new question
        useEffect(() => {
            setAnswered(false);
        ...
        // Timer
        useEffect(() => {
            if (answered || isPausedTimer) return;

            const timer = setInterval(() => {
                setTimeLeft((prev:number) => {
                    if (prev <= 1) {
                        clearInterval(timer);
                        handleSubmit(); // auto-send when timer runs out
                        return 0;
                    }
                    return prev - 1;
                });
                    ...
                    {!reviewMode && (
                        <View style={styles.column}>
                            ...
                            {shuffledRight.map((right) => {
                                const leftMatched = Object.keys(matches).find(
                                    (l) => matches[l] === right
                                );
